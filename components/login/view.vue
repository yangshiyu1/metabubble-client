...
<template>
    ...
    <dialog ref="loginForm" class="modal">
        <div style="background-color: #0d1b2a;" class="modal-box  max-w-xl">
            <form method="dialog">
                <button style="border-color: aliceblue; color: #fff;" class="btn btn-sm btn-circle btn-ghost absolute right-2 top-2" @click="resetForm">✕</button>
            </form>
            <div class="tabs flex justify-center items-center w-full pb-3">
                <button id="login" class="btn  tab tab-lifted tab-lg text-xl font-bold"
                    :class="{ 'tab-active': activeTab === 'login' }" @click="setActiveTab('login')" v-if="activeTab === 'login'">登录</button>
                ...
                <button id="signup" class="tab  text-black  tab-lifted tab-lg text-xl font-bold"
                    :class="{ 'tab-active': activeTab === 'signup' }" @click="setActiveTab('signup')" v-if="activeTab === 'signup'">注册</button>
            ...
            <div class="flex justify-center items-center">
                <button v-if="activeTab === 'login'" class="bg-[#F7AB0A] p-2 rounded transition-all hover:bg-[#FFC857] btn btn-wide btn-circle font-bold text-lg" @click="login">
                    <span>登录</span>
                </button>
                <button v-else class="bg-[#F7AB0A] p-2 rounded transition-all hover:bg-[#FFC857] btn btn-wide btn-circle font-bold text-lg">
                    <span>注册</span>
                </button>
            </div>
        </div>
