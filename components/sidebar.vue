...
" class="rounded-l-xl md:w-64 w-24 bg-base-200 top-0 h-screen fixed flex flex-col px-4  border-base-300">
      ...
      <NuxtLink class="btn btn-ghost justify-start text-xl font-normal items-center rounded-full py-4 h-auto"
        ...
        <span style="letter-spacing: 0.3em;" v-if="(item.path === ('/' + useCenter.currentPath.split('/')[1]))" class=" ml-3 tracking-wide px-3 hidden md:block font-bold">{{
          item.name }}</span>
        <span style="letter-spacing: 0.3em;" v-else class="ml-3 px-3 hidden md:block tracking-wider">{{ item.name }}</span>
      </NuxtLink>
